...
class ActiveDirectoryMgmt:
    ...
    def BindADServer(self):
        ...
        server = Server(self.ad_server_ip, port=636, use_ssl=True,
                        tls=tls_config, get_info=ALL)
        con = Connection(server, user=self.ad_bind_userdn,
    ...
    def search_adusers_information(self, SEARCH_USERS):
        ...
        try:
            ...
            conn.search(self.ad_base_dn, filters,
                        attributes=self.ad_attributes_list, paged_size=5,
        ...
        for ex in rs_respone:
            user_data = {}
            for i in self.ad_attributes_list:
    ...
    def search_adusers_mini_information(self, SEARCH_USERS):
        ...
        attributes_list = ['displayname', 'Distinguishedname',
                           'accountexpires', 'userPrincipalName', 'userAccountControl']
    ...
    def search_ad_users_group_dn(self, SERCH_TYPE, SEARCH_NAME):
        ...
        ad_attributes_list = ['Distinguishedname']

        DN_list = []
        for ex in entry:
            user_data = {}
            for i in ad_attributes_list:
                user_data['DN'] = ex['attributes'][i]
            DN_list.append(user_data)
    ...
    def create_user(self, USER_BASE_DN, NEW_USER, NEW_USER_PASWORD, DESCRIPTION):
        try:
            ...
            conn.add(temp_user_dn, 'user', {'givenName': NEW_USER, 'description': "Create Time - " + time.ctime() + " " + DESCRIPTION,
                                            'samaccountname': NEW_USER, 'userPrincipalName': NEW_USER + '@' + self.ad_domain_name})
    ...
    def create_group(self, GROUP_BASE_DN, NEW_GROUP, DESCRIPTION):
        try:
            ...
            return {'desctiption': 'CreateGroupErrors', 'groupname': NEW_GROUP, 'groupdn': temp_group_dn, 'result': conn.result}
